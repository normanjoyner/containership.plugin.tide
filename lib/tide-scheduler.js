...
module.exports = {
    ...
    initialize: function(core) {
        this.core = core;
        this.myriad_key = this.core.constants.myriad.TIDE_JOBS;

        const on_defrost = (err) => {
            if(err) {
                this.core.loggers["tide-scheduler"].log("error", ["Unable to restore tide jobs from myraid-kv:", JSON.stringify(err)].join(" "));
            } else {
    ...
    schedule_job: function(config) {
        if(_.has(this.jobs, config.id)) {
            this.jobs[config.id].schedule(() => {
                this.core.loggers["tide-scheduler"].log("verbose", ["Deploying tide application", config.application.id, "to", config.instances, "containers."].join(" "));
    ...
    defrostJobs: function(fn) {
        this.core.cluster.myriad.persistence.get(this.myriad_key, (err, savedJobs) => {
            if(err) {
                ...
                _.forEach(savedJobs, (j) => {
                    this.jobs[j.id] = new Job(j);
                });
